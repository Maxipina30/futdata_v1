"""
Scraper de SofaScore para partidos de Colo-Colo.

Descarga lineups e incidents de cada partido terminado de Colo-Colo
en la temporada activa de la Primera División de Chile.

Uso:
    venv312/Scripts/python.exe apps\\liga_chilena\\colo_colo_plus_minus\\src\01_scrape.py
"""
...
OUT_DIR = BASE_DIR / "apps" / "liga_chilena" / "colo_colo_plus_minus" / "data" / "raw"
...
async def fetch_match_data(context, event, referer):
    ...
    home = (event.get("homeTeam") or {}).get("name", "?")
    away = (event.get("awayTeam") or {}).get("name", "?")
    print(f"  Partido {eid}: {home} vs {away}")
    ...
    (match_dir / "meta.json").write_text(json.dumps(meta, ensure_ascii=False, indent=2), encoding="utf-8")

    if lineups:
        (match_dir / "lineups.json").write_text(json.dumps(lineups, ensure_ascii=False, indent=2), encoding="utf-8")
    else:
        print(f"    Sin lineups")

    if incidents:
        (match_dir / "incidents.json").write_text(json.dumps(incidents, ensure_ascii=False, indent=2), encoding="utf-8")
    else:
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        print(f"  Total eventos en la liga: {len(all_events)}")

        colo_events = [e for e in all_events if is_colo_colo_match(e)]
        finished = [e for e in colo_events if is_finished(e)]
        print(f"  Partidos de Colo-Colo: {len(colo_events)} ({len(finished)} terminados)")
