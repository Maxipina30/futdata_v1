...
Uso:
    .runtime\python312\Scripts\streamlit.exe run analysis/liga_chilena_pm/dashboard.py
"""
...
from colo_xi import show_colo_xi_tab
...
# ╔══════════════════════════════╗
# ║  Tab 5: Forma reciente      ║
# ╚══════════════════════════════╝
with tab5:
    st.subheader("📅 Forma reciente — +/- por partido")

    form_df = detail_all.copy()
    if team_sel != "Todos los equipos":
        form_df = form_df[form_df["team"] == team_sel]
    if pos_code:
        pid_pos = pm_all[pm_all["position"] == pos_code]["player_id"].unique()
        form_df = form_df[form_df["player_id"].isin(pid_pos)]

    n_matches = st.slider("Últimos N partidos", 3, 15, 8)
    min_min_form = st.slider("Minutos mínimos por partido", 30, 90, 45, step=15,
                             key="form_min")

    form_df = form_df[form_df["minutes"] >= min_min_form]

    # Pivotear: jugador × fecha
    form_df["date"] = pd.to_datetime(form_df["date"])
    form_df = form_df.sort_values("date")
    last_dates = sorted(form_df["date"].unique())[-n_matches:]
    form_df = form_df[form_df["date"].isin(last_dates)]

    pivot = form_df.pivot_table(index="player", columns="date", values="pm",
                                aggfunc="sum")
    pivot = pivot.dropna(thresh=2)  # al menos 2 partidos con datos

    # Ordenar por suma de +/- reciente
    pivot = pivot.loc[pivot.sum(axis=1).sort_values(ascending=False).index]
    pivot = pivot.head(30)

    if pivot.empty:
        st.info("Sin datos suficientes con los filtros aplicados.")
    else:
        x_labels = [str(d.date()) for d in pivot.columns]
        fig = go.Figure(go.Heatmap(
            z=pivot.values,
            x=x_labels,
            y=pivot.index.tolist(),
            colorscale=[[0, RED], [0.5, "#2e3f52"], [1, GREEN]],
            zmid=0,
            text=pivot.values.astype(int).astype(str),
            texttemplate="%{text}",
            hovertemplate="Jugador: %{y}<br>Fecha: %{x}<br>+/-: %{z}<extra></extra>",
            colorbar=dict(title="+/-", tickfont_color="white"),
        ))
        fig.update_layout(
            **PLOTLY_THEME,
            height=max(400, len(pivot) * 26),
            title=f"Heatmap +/- — últimos {n_matches} partidos  (top 30 jugadores)",
